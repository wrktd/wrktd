...
type ProductDef = { id: string; label: string; size: string; aspect: number; bg: string; weaveBg?: string; radius: string; shadow?: string; };
...
const PRODUCTS: ProductDef[] = [
  ...
  { id: "tapestry", label: "Wall Tapestry", size: "36×48 in", aspect: 3/4, bg: "#181818",
    radius: "3px", shadow: "0 8px 40px rgba(0,0,0,0.7), 0 0 0 1px rgba(255,255,255,0.06)" },
    ...
    <div className="flex flex-col gap-3">
      <div className="relative w-full" style={{ paddingBottom: `${(1 / product.aspect) * 100}%` }}>
        <div className="absolute inset-0 overflow-hidden"
          ...
          <div className="absolute inset-0 pointer-events-none" style={{ background: "linear-gradient(135deg, rgba(255,255,255,0.04) 0%, transparent 50%, rgba(0,0,0,0.2) 100%)" }} />
        </div>
      </div>
      <div>
    ...
    <section className="glass overflow-hidden">
      <div className="relative z-10 max-w-5xl mx-auto px-6 md:px-10 pt-28 pb-28">
        ...
        <div className="grid gap-8 mb-12" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))" }}>
          {PRODUCTS.map((p) => (
            <ProductCard key={p.id} product={p} imageUrl={(imageUrl || loading) ? imageUrl : null} loading={loading} />
          ))}
        </div>
